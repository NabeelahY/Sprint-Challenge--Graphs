...
def bfs(player, roomQueue):
    # get player current room id
    currentRoom = rooms[player.currentRoom.id]
    ...
    for d in currentRoom:
        ...
        if currentRoom[d] == '?':
            ...
            unexploredExits.append(d)
        # if unexplored
    if len(unexploredExits) > 0:
        roomQueue.enqueue(unexploredExits[0])
    else:
        unexplored_path = goCheckOtherRooms(player, roomQueue)

        currentPath = player.currentRoom.id

        for path in unexplored_path:
            for d in rooms[currentPath]:
                if rooms[currentPath][d] == path:
                    roomQueue.enqueue(d)
                    currentPath = path
                    break


def goCheckOtherRooms(player, roomQueue):
    ...
    while q.size() > 0:
        ...
        if last_item not in visited:
            ...
            for exit in rooms[last_item]:
                if rooms[last_item][exit] == '?':
                    return node
                else:
                    new_path = list(node)
                    new_path.append(rooms[last_item][exit])
                    q.enqueue(new_path)
    return []
...
for d in player.currentRoom.getExits():
    currentRoom[d] = '?'
rooms[player.currentRoom.id] = currentRoom

roomQueue = Queue()

bfs(player, roomQueue)

while roomQueue.size() > 0:
    start_room = player.currentRoom.id
    next_move = roomQueue.dequeue()
    player.travel(next_move)
    traversalPath.append(next_move)

    end_room = player.currentRoom.id
    rooms[start_room][next_move] = end_room

    if end_room not in rooms:
        rooms[end_room] = {}

        for direction in player.currentRoom.getExits():
            rooms[end_room][direction] = '?'

    rooms[end_room][inverse_directions[next_move]] = start_room
    if roomQueue.size() == 0:
        bfs(player, roomQueue)
...
#     rooms[player.currentRoom.id] = se

# worldGraph.add_vertex(player.currentRoom.id)
# # vertices[player.currentRoom.id] = set()
# for d in player.currentRoom.getExits():
#     worldGraph.add_edge(player.currentRoom.id, d)

# print(player.currentRoom.getExits())

# print(worldGraph.vertices)
print(rooms)
